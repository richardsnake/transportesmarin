...
?>\n";
	...
	{	
		...
		{
			$consult = "insert into articulo(Descripcion, Peso, Flete, TipoArticulo, Remitente, Destinatario, TipoEntrega, TipoPago, EstadoPago, activo, Viaje_CodViaje, Cliente_CodigoCliente, Comprobante_Codigo, usuario) values('".$_POST["desc"]."', '".$_POST["peso"]."', '".$_POST["flete"]."', '".$_POST["tipoA"]."', '".$_POST["rem"]."', '".$_POST["dest"]."', '".$_POST["tipoE"]."', '".$_POST["tipoP"]."', '".$_POST["estadoP"]."', 1, '".$_POST["codViaje"]."', '".$_POST["codCliente"]."', '".$_POST["comp"]."', '".$_SESSION["usuario"]."');";
			$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);			
			$bd->conectar();
		...
		else if("buscarComp"==$op)
		{
			...
			{			
				...
				$cad.=$cliente."*".$serie."*".$numero."*".$total."*".$fecha."*".$tipo."*".$saldo."*".$dirO."*".$dirD."*".$nroGuia;				
				print($cad);	
			}
		}
		...
		else if("regGastoViaje"==$op)
		{
			...
			if($_POST["band"]=="1") //Tipo combustible
			{
				...
				$consult = "insert into gastos(monto, fecha, tipo, razonSocial, nroComprobante, precioGalon, nroGalones, activo, Viaje_codViaje, usuario) values($monto, '$fecha', 'combustible', '$razonS', '$nroComp', $precGalon, $nroGalones, 1, $codV, '".$_SESSION["usuario"]."');";
				$result = $bd->crearConsulta($consult);
				print("exito");				
			}
			else
			{
				...
				$desc = $_POST["desc"];
				$monto = $_POST["monto"];
				$consult = "insert into gastos(monto, fecha, descripcion, tipo, razonSocial, nroComprobante, activo, Viaje_codViaje, usuario) values($monto, '$fecha', '$desc', 'otros', '$razonS', '$nroComp', 1, $codV, '".$_SESSION["usuario"]."');";
				$result = $bd->crearConsulta($consult);
				print("exito");	
			}				
		}	
