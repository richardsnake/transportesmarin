<?php
	...
	switch($opc)
	{
		...
		case 'actualizarPagoViaje':	actualizarPagoViaje();
									break;
									
		case 'actualizarGastosViaje':	actualizarGastosViaje();
										break;
													
		case 'eliminarPagoViaje':	eliminarPagoViaje();
									break;
		
		case 'eliminarGastosViaje': 	eliminarGastosViaje();
										break;
		...
		case 'actualizarVehiculo': actualizarVehiculo();
									break;					
		case 'actualizarCarreta':	actualizarCarreta();
									break;		
		...
		/*case 'prodTrabajador':	guardarProductoTrabajador();
								break;*/
		case 'prodTrabajador':	guardarProductoTrabajadorNew();
								break;								
								
		/*case 'regProducto':	registrarProducto();
							break;	*/							
		case 'descuento':	guardarDescuentoNew();
							break;	
							
		case 'ruta':	guardarRuta();
						break;	
						
		case 'pagoPersonal': guardarPagoPersonalNew();
								break;		

		case 'comprovCompra':	guardarComprobanteCompra();
								break;
		case 'detalleEntrega':	detalleEntrega();
								break;	
		case 'crearCuentaGasto':	crearCuentaGasto();
									break;		
		case 'eliminarCuentaGasto': eliminarCuentaGasto();
									break;					
		case 'registrarIGV':		registrarIGV();
									break;											
	...
	function loggear()
	{
		...
		if(!$band)
		{
			...
			if(!$band)
			{
				$consulta = "select usuario, password, Cliente_CodigoCliente as cod from usuario;";
				$result = $bd->crearConsulta($consulta);
				while($registro = mysql_fetch_object($result))
				{
					...
					if($cuenta==$usuario && $password==$clave)
					{
						...
						header("Location: usuario.php?cod=$cod");
						die;				
					}
				}
			}			
...
?>	
	...
	function registrarIGV()
	{
		$porcentaje=$_POST["txtPorcentaje"];
		$valor=$_POST["txtValor"];
		$bd=new BaseDatos(_SERVIDOR, _BASEDATOS, _USUARIO, _PASSWORD);
		$bd->conectar();
		$result=$bd->crearConsulta("insert into igv(procentaje, valor) values('$porcentaje', '$valor')");			
		if($result)
		{
			?>	
		<script language="javascript" type="text/javascript">
			alert("El IVG fue registrado correctamente");
	...
	function detalleEntrega()
	{
		...
		if(count($entregar)>0)
		{	
			if(count($almacenar)<1)
			{
			//echo("Cambiar estado de entrega en el comprobante y poner la fecha actual y tbm de los articulos entregados");		
				$sql1="update comprobante set estadoEntrega='Entregado', fechaEntrega='".date('Y-m-d')."', observacion = '".$_POST["txtObservacion"]."' where codigo=".$cod;
				$bd->crearConsulta($sql1);
				echo($sql1);
				while($i<count($entregar))
				{
					$sql2="update articulo set estadoEntrega='Entregado' where CodigoArticulo=".$entregar[$i];
					echo($sql2);
				...
				$sql1="update comprobante set estadoEntrega='Pendiente', fechaEntrega='".date('Y-m-d')."', observacion = '".$_POST["txtObservacion"]."' where codigo=".$cod;
				$bd->crearConsulta($sql1);
				echo($sql1);
				while($i<count($entregar))
				{
			...
			{*/
				//echo("Ademas, poner a los  articulos en estado de almacen y su fecha correspondiente");
				$i=0;
				while($i<count($almacenar))
				{
					$sql3="update articulo set estadoEntrega='En almacen', fechaAlmacen='".date('Y-m-d')."' where CodigoArticulo=".$almacenar[$i];
					$bd->crearConsulta($sql3);
					echo($sql3);
					$i++;
				}
			}
		...
		else 
		{
			if(count($almacenar)>0)
			{
				//echo("Cambiar estadoEntrega a articulo a almacen y la fecha correspondiente y ademas actualizar la observacion en el comprobante");
				$i=0;
				$sql4="update comprobante set estadoEntrega='Pendiente', observacion='".$_POST["txtObservacion"]."' where codigo=".$cod;
				$bd->crearConsulta($sql4);
				echo($sql4);
				while($i<count($almacenar))
				{
					$sql5="update articulo set estadoEntrega='En Almacen', fechaAlmacen='".date('Y-m-d')."' where CodigoArticulo=".$almacenar[$i];
					echo($sql5);
					$bd->crearConsulta($sql5);
					$i++;
				}
			}
			else
			{
				...
				?>	
		<script language="javascript" type="text/javascript">
			//alert("¡ Entrega de Articulos Actualizado exitosamente !");
			location.href="administrador.php";
		</script>
		<?php		
				//echo("regresar al documento anterior");
			}
		}
		$bd->cerrarConexion();
		?>	
	...
	function crearCuentaGasto()
	{
		$nombre=$_POST["txtNombre"];
		$bd=new BaseDatos(_SERVIDOR, _BASEDATOS, _USUARIO, _PASSWORD);
		$bd->conectar();
		$result=$bd->crearConsulta("insert into cuentasgastos(nombre) values('$nombre');");
		if($result)
		{
			?>	
		<script language="javascript" type="text/javascript">
			alert("¡ La nueva Cuenta de gasto se ah creado exitosamente !");
			location.href="administrador.php";
		</script>
		<?php
		}
	...
	function eliminarCuentaGasto()
	{
		$nombre=$_POST["cuenta"];
		$bd = new BaseDatos(_SERVIDOR, _BASEDATOS, _USUARIO, _PASSWORD);
		$bd->conectar();
		$result=$bd->crearConsulta("delete from cuentasgastos where nombre='$nombre'");
		if($result)
		{
			?>	
		<script language="javascript" type="text/javascript">
	...
	function actualizarPagoViaje()
	{
		$cod=$_POST["textCodigo"];
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();
		$consulta = "update pagos set descripcion='$descripcion', fecha=$fecha, monto=$monto where codigoPago=$cod;";
		$bd->crearConsulta($consulta);
		?>	
		<script language="javascript" type="text/javascript">
			alert("¡ Pago de viaje Actualizado exitosamente !");
			location.href="administrador.php";
		</script>
		<?php
	}
	function actualizarGastosViaje()
	{
		$cod=$_POST["textCodigo"];
		$descripcion = $_POST["txtDescripcion"];
		$fecha = $_POST["txtFecha"];
		$monto= $_POST["txtMonto"];
		$tipo = $_POST["txtTipo"];
		$razonS= $_POST["txtRazonS"];
		$nroComprobante= $_POST["txtNroComp"];
		$precioGalon= $_POST["txtPrecioGalon"];
		$nroGalones= $_POST["txtNroGalones"];
		//print($cod);
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();
		$consulta = "update gastos set descripcion='$descripcion', fecha=$fecha, monto=$monto, tipo='$tipo', razonSocial='$razonS', nroComprobante='$nroComprobante', precioGalon=$precioGalon, nroGalones=$nroGalones where codigo=$cod;";
		$bd->crearConsulta($consulta);
		?>	
		<script language="javascript" type="text/javascript">
			alert("¡ Gasto de viaje Actualizado exitosamente !");
			location.href="administrador.php";
		</script>
		<?php
	}	
	function eliminarPagoViaje()
	{
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();
		$consulta = "DELETE FROM pagos WHERE codigoPago=$cod;";
		$bd->crearConsulta($consulta);
		?>	
		<script language="javascript" type="text/javascript">
			alert("¡ Pago de Viaje eliminado exitosamente !");
			location.href="administrador.php";
		</script>
		<?php
	}
	
	function eliminarGastosViaje()
	{
		$cod=$_POST["textCodigo"];
		//print($cod);
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();
		$consulta = "DELETE FROM gastos WHERE codigo=$cod;";
		$bd->crearConsulta($consulta);
		?>	
		<script language="javascript" type="text/javascript">
			alert("¡ Pago de Viaje eliminado exitosamente !");
	...
	function actualizarComp()
	{
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();				
		$consulta = "update comprobante set TipoComprobante='$tipo', Serie='$serie', Numero='$numero', Fecha='$fecha', Detraccion='$detrac', total='$total', direccionOrigen='$origen', direccionDestino='$destino', nGuiaRemision='$guiaR', usuario='".$_SESSION["usuario"]."' where $campo='$codComp';";
		$bd->crearConsulta($consulta);	
	...
	function anularComp()
	{
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();				
		$consulta = "update comprobante set activo=0 where Numero='$codComp';";
		$bd->crearConsulta($consulta);		
		?>	
		<script language="javascript" type="text/javascript">
	...
	function actualizarVehiculo()
	{
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();		
		$consulta = "UPDATE vehiculo SET Marca='$marca', Modelo='$modelo', nCertificado='$nCertificado', nRegistro='$nRegistro', Tara=$tara, pesoBruto=$pesoBruto, cargaUtil=$pesoBruto-$tara, Anho=$anho, tipoVehiculo='$tVehiculo', TipoCombustible='$tCombustible', NEjes=$nEjes, usuario='".$_SESSION["usuario"]."' WHERE placa='$placa';";
		$result = $bd->crearConsulta($consulta);
		?>
		<script language="javascript" type="text/javascript">
	...
	function actualizarCarreta()
	{
		...
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();		
		$consulta = "UPDATE carreta SET nInscripcion='$nCertificado', nRegistro='$nRegistro', Tara=$tara, pesoBruto=$pesoBruto, cargaUtil=$pesoBruto-$tara,  nEjes=$nEjes, usuario='".$_SESSION["usuario"]."' WHERE placa='$placa';";
		$result = $bd->crearConsulta($consulta);
		?>
		<script language="javascript" type="text/javascript">
			alert("! El Carreta se actualizo correctamente !");
			location.href="administrador.php";
		</script>
		<?php
	}
	...
	function registrarComprobante()
	{
		...
		$consult = "update comprobante set Fecha='".date('Y-m-d')."', Serie='$serie', Detraccion='$detraccion', total='$total', direccionOrigen='$dirO', direccionDestino='$dirD', destino='$dest', docCli='$docCli', nGuiaRemision='$nGR', EstadoPago='$estadoP',igv='$igv', activo=1, usuario='".$_SESSION["usuario"]."' where codigo='$comp';";
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();			
	...
	function regArticulo()
	{
		...
		{
			$consult = "select CodigoCliente, TipoCliente, RazonSocial, Nombres, ApellidoPaterno, ApellidoMaterno from cliente where DNI='$dni_rucR' or RUC='$dni_rucR';";
			$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
			$bd->conectar();			
			...
			{
				$rem = $reg->Nombres." ".$reg->ApellidoPaterno." ".$reg->ApellidoMaterno;
			}
		}
		...
		{
				$consult = "select CodigoCliente, Nombres, ApellidoPaterno, ApellidoMaterno, RazonSocial, TipoCliente from cliente where DNI='$dni_rucD' or RUC='$dni_rucD';";
				$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
				$bd->conectar();			
				$result = $bd->crearConsulta($consult);				
				$reg  = mysql_fetch_object($result);
				...
				{
					$dest = $reg->Nombres." ".$reg->ApellidoPaterno." ".$reg->ApellidoMaterno;
				}	
		}
				
		
		$consult = "insert into comprobante(Sucursal_codigoSucursal, Cliente_CodigoCliente, usuario, estadoEntrega) values($codSucursal, $claveClienteR, '".$_SESSION["usuario"]."','Por entregar');";
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();			
		$bd->crearConsulta($consult);	
		$claveComp = mysql_insert_id();		
		$desc=$bultos."-".$desc;				
		$consult = "insert into articulo(Descripcion, Peso, Flete, TipoArticulo, Remitente, Destinatario, TipoEntrega, TipoPago, EstadoPago, estadoEntrega, activo, Viaje_CodViaje, Cliente_CodigoCliente, Comprobante_Codigo, usuario) values('$desc', $peso, $flete, '$tipoA', '$rem', '$dest', '$tipoE', '$tipoP', '$estadoP', 'Por entregar', 1, '$codViaje', '$claveClienteR', '$claveComp', '".$_SESSION["usuario"]."');";
		$bd = new BaseDatos(_SERVIDOR,_BASEDATOS,_USUARIO,_PASSWORD);
		$bd->conectar();			
		$bd->crearConsulta($consult);	
//		header("Location: formRegistrarNuevoArt.php?op=agregarArt&claveComp=$claveComp&Rem=$claveClienteR&Dest=$claveClienteD&CodViaje=$codViaje&CodCliente=$claveClienteR");	
	...
	function guardarProductoTrabajadorNew()
	{
		$dni = $_POST["dni"];
	...
	function guardarDescuentoNew()
	{
		...
		$bd->crearConsulta($consulta);
		$bd->cerrarConexion();
		header("Location: actaDescuentoPersonal.php?dni=$dni&monto=&monto=$monto&motivo=$motivo");		
	}
	...
	function guardarDescuento()
	{
		...
		<script language="javascript" type="text/javascript">
			...
			//location.href="actaPagoPersonal.php?dni=$dni&monto=&monto=%monto&motivo=$motivo";
		</script>
		<?php
		
	}
	
	function guardarPagoPersonalNew()
	{
		$dni = $_POST["dni"];
		$monto = $_POST["textMonto"];
		$monto = number_format($monto, 2, '.', '');
		...
		$bd->crearConsulta($consulta);
		$bd->cerrarConexion();
		header("Location: actaPagoPersonal.php?dni=$dni&monto=&monto=$monto&motivo=$motivo");
		?>
		<script language="javascript" type="text/javascript">
